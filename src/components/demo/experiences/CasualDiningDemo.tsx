...
import {
  ...
  Bell, MessageCircle, ThumbsUp, Sparkles, Wine,
} from 'lucide-react';
...
export const SCREEN_INFO: Record<Screen, { emoji: string; title: string; desc: string }> = {
  ...
  'waitlist': { emoji: '⏱️', title: 'Lista de Espera', desc: 'Posição em tempo real com notificação push.' },
...
interface MenuItem {
  id: string; name: string; price: number; emoji: string; cat: string;
  desc: string; popular?: boolean; kids?: boolean; vegetarian?: boolean;
  allergens?: string[]; prepTime?: number;
}

const MENU: MenuItem[] = [
  { id: 'm1', name: 'Lasanha Bolonhesa', price: 52, emoji: '🍝', cat: 'Massas', desc: 'Camadas de massa fresca com ragu bolonhês e bechamel', popular: true, prepTime: 25, allergens: ['glúten', 'lactose'] },
  { id: 'm2', name: 'Pizza Pepperoni', price: 58, emoji: '🍕', cat: 'Pizzas', desc: 'Massa artesanal, pepperoni importado e mussarela', popular: true, prepTime: 18, allergens: ['glúten', 'lactose'] },
  { id: 'm3', name: 'Risoto de Camarão', price: 72, emoji: '🦐', cat: 'Especiais', desc: 'Arroz arbóreo, camarões grelhados e açafrão', prepTime: 30, allergens: ['crustáceos', 'lactose'] },
  { id: 'm4', name: 'Filé à Parmegiana', price: 65, emoji: '🥩', cat: 'Carnes', desc: 'Filé empanado com molho pomodoro e queijo gratinado', popular: true, prepTime: 22, allergens: ['glúten', 'lactose'] },
  { id: 'm5', name: 'Salada Caesar', price: 38, emoji: '🥗', cat: 'Saladas', desc: 'Alface romana, croutons, parmesão e molho caesar', vegetarian: true, prepTime: 8, allergens: ['glúten', 'lactose'] },
  { id: 'm6', name: 'Tiramisù', price: 28, emoji: '🍰', cat: 'Sobremesas', desc: 'Clássico italiano com café e mascarpone', prepTime: 5, allergens: ['glúten', 'lactose', 'ovos'] },
  { id: 'm7', name: 'Mini Pizza Margherita', price: 25, emoji: '🍕', cat: 'Kids', desc: 'Tamanho perfeito para os pequenos', kids: true, prepTime: 12, allergens: ['glúten', 'lactose'] },
  { id: 'm8', name: 'Nuggets com Batata', price: 22, emoji: '🍗', cat: 'Kids', desc: 'Nuggets caseiros com batata frita', kids: true, prepTime: 15, allergens: ['glúten'] },
  { id: 'm9', name: 'Macarrão com Queijo', price: 20, emoji: '🧀', cat: 'Kids', desc: 'Mac & cheese cremoso', kids: true, prepTime: 10, allergens: ['glúten', 'lactose'] },
  { id: 'm10', name: 'Bruschetta', price: 26, emoji: '🍞', cat: 'Entradas', desc: 'Tomate fresco, manjericão e azeite', vegetarian: true, prepTime: 8, allergens: ['glúten'] },
];

interface OrderItem { item: MenuItem; qty: number; who: string; notes?: string; }

const PEOPLE = [
  { id: 'p1', name: 'Você', emoji: '👤', color: 'bg-primary' },
  { id: 'p2', name: 'Maria', emoji: '👩', color: 'bg-pink-500' },
  { id: 'p3', name: 'João', emoji: '👨', color: 'bg-blue-500' },
  { id: 'p4', name: 'Sofia', emoji: '👧', color: 'bg-purple-500', isKid: true },
];
...
export const CasualDiningDemo: React.FC<Props> = ({ onNavigate, screen }) => {
  ...
  const [waitlistDrinks] = useState([
    { name: 'Caipirinha', price: 22, emoji: '🍹' },
    { name: 'Cerveja Artesanal', price: 18, emoji: '🍺' },
    { name: 'Suco Natural', price: 12, emoji: '🧃' },
    { name: 'Porção de Pão de Alho', price: 16, emoji: '🧄' },
  ]);
      ...
      return (
        <div className="px-5 pb-4">
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-hide mb-4">
            {['Kids 👶', 'Pet Friendly 🐶', 'Ao ar livre ☀️', 'Estacionamento 🅿️', 'Acessível ♿'].map((f, i) => (
      ...
      return (
        <div className="px-5 pb-4">
          ...
          <div className="p-3 rounded-xl bg-primary/5 border border-primary/10 mb-4">
            <p className="text-xs font-semibold mb-1">🕐 Status Agora</p>
          ...
          <div className="space-y-3">
            <button onClick={() => onNavigate('waitlist')} className="w-full p-5 rounded-2xl border-2 border-primary bg-primary/5 text-left">
              ...
              <div className="grid grid-cols-3 gap-2">
                {['🍹 Pedir drinks', '📱 Notificação', '📋 Ver cardápio'].map(f => (
                  <span key={f} className="text-[10px] text-center py-1.5 rounded-lg bg-primary/5 text-primary">{f}</span>
                ))}
              </div>
            </button>
      ...
      return (
        <div className="px-5 pb-4">
          ...
          <div className="p-4 rounded-xl bg-gradient-to-r from-primary/10 to-accent/10 border border-primary/20 mb-4">
            <p className="text-sm font-bold mb-3">👨‍👩‍👧‍👦 Modo Família Ativado</p>
            <div className="space-y-2">
              {[
                { icon: '🍕', label: 'Cardápio Kids em destaque', desc: 'Itens especiais para crianças primeiro' },
                { icon: '🪑', label: 'Cadeirão reservado', desc: 'Já preparamos tudo para vocês' },
                { icon: '🎨', label: 'Kit de atividades', desc: 'Jogos e colorir na mesa' },
                { icon: '⚡', label: 'Pratos kids primeiro', desc: 'Crianças comem antes, sem espera' },
                { icon: '🚫', label: 'Alerta de alérgenos', desc: 'Itens com alérgenos infantis destacados' },
              ].map((f, i) => (
          ...
          <h3 className="font-semibold text-sm mb-2">👧 Quem são as crianças?</h3>
          <div className="space-y-2 mb-4">
            {[
    ...
    case 'family-activities':
      return (
        <div className="px-5 pb-4">
          ...
          <div className="space-y-3">
            {[
              { name: 'Colorir na Mesa', desc: 'Kit de lápis e desenhos da Cantina', emoji: '🎨', active: true },
              { name: 'Quiz da Pizza', desc: 'Jogo interativo no tablet da mesa', emoji: '🧩', active: true },
              { name: 'Caça ao Tesouro', desc: 'Encontre 5 itens escondidos no restaurante', emoji: '🗺️', active: false },
              { name: 'Chef Mirim', desc: 'Monte sua própria mini pizza (30 min)', emoji: '👨‍🍳', active: true },
            ].map((act, i) => (
          ...
          <div className="space-y-2">
            {filtered.map(item => (
              <button key={item.id} onClick={() => { setSelectedItem(item); onNavigate('item-detail'); }} className="w-full flex items-center gap-3 p-3 rounded-xl border border-border bg-card text-left">
                <div className="w-12 h-12 rounded-lg bg-muted/50 flex items-center justify-center text-2xl">{item.emoji}</div>
    ...
    case 'item-detail':
      ...
      return (
        <div className="px-5 pb-4">
          ...
          <div className="text-center mb-4">
            <div className="w-24 h-24 rounded-2xl bg-muted/50 flex items-center justify-center mx-auto mb-3">
          ...
          {groupedByPerson.filter(p => p.items.length > 0).map(person => (
            <div key={person.id} className="mb-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                {'isKid' in person && person.isKid && <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-purple-500/10 text-purple-500">👶 Kids</span>}
              ...
              {person.items.map((order, i) => (
                <div key={i} className="flex items-center gap-3 py-2 pl-8 border-b border-border/50">
                  <span className="text-lg">{order.item.emoji}</span>
                  <div className="flex-1">
      ...
      return (
        <div className="px-5 pb-4">
          ...
          <div className="grid grid-cols-2 gap-2 mb-5">
            {[
              { id: 'mine' as const, name: 'Meus Itens', desc: `R$ ${myTotal + Math.round(myTotal * 0.1)}`, icon: '👤' },
              { id: 'equal' as const, name: 'Partes Iguais', desc: `R$ ${equalSplit} cada`, icon: '⚖️' },
              { id: 'by-item' as const, name: 'Por Item', desc: 'Selecione os itens', icon: '🧾' },
              { id: 'fixed' as const, name: 'Valor Fixo', desc: 'Defina o valor', icon: '💵' },
            ].map(mode => (
            ...
            <div className="p-4 rounded-xl bg-card border border-border mb-4">
              ...
              {orders.filter(o => o.who === 'Você').map((o, i) => (
                <div key={i} className="flex justify-between text-sm py-1 border-b border-border/50 last:border-0">
                  <span>{o.item.emoji} {o.item.name}</span><span className="font-semibold">R$ {o.item.price}</span>
                </div>
              ))}
    ...
    case 'review':
      return (
        <div className="px-5 pb-4">
          ...
          {[
            { label: 'Comida', key: 'food' as const, emoji: '🍝' },
            { label: 'Serviço', key: 'service' as const, emoji: '👨‍🍳' },
            { label: 'Ambiente', key: 'ambiance' as const, emoji: '🏠' },
          ].map(cat => (
